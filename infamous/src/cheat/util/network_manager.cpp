...
namespace Menu::Network {
	...
	int GetLauncherIndexFromType(Rage::joaat_t scriptHash) {
		for (int i = 0; i < launcher_scripts.size(); i++)
			if (launcher_scripts[i] == scriptHash)
				return i;
		return -1;
	}
	...
	template<typename T>
	void SetBit(T* address, int pos)
	{
		*address |= 1 << pos;
	}

	void StartLauncherScript(int scriptId) {
		static auto CheckPlayerState = [](GtaThread* launcher, int state) -> bool {
			...
			Menu::GetPlayerManager()->Foreach([=](Menu::PlayerVars& Player) {
				if (((CGameScriptHandlerNetComponent*)launcher->m_net_component)->is_player_a_participant(Player.m_NetGamePlayer)) {
					if (*Menu::ScriptLocalStack(launcher->m_stack, 236).At(Player.m_ID, 3).At(2).As<int*>() == state) {
						set = true;
						return;
					}
				}
		...
		if (auto launcher = FindScriptThread(RAGE_JOAAT("am_launcher"))) {
			if (!ForceScriptHost(RAGE_JOAAT("am_launcher")))
			{
				Menu::Notify::stacked("Failed To Launch Script");
				return;
			}

			launcher->m_context.m_state = Rage::eThreadState::Halted;

			if (CheckPlayerState(launcher, 5)) {
				for (int i = 0; CheckPlayerState(launcher, 5); i++) {
					if (i > 200)
						break; // 3F) Timeout

					Menu::Global(2698947).At(3).At(1).As<int>() = 0;
					Menu::Global(2698947).At(2).As<int>() = 6;
				}
			}

			if (CheckPlayerState(launcher, 6)) {
				for (int i = 0; CheckPlayerState(launcher, 6); i++)	{
					if (i > 200)
						break; // 4F) Timeout

					Menu::Global(2698947).At(3).At(1).As<int>() = 0;
					Menu::Global(2698947).At(2).As<int>() = 7;
				}
			}

			if (CheckPlayerState(launcher, 7)) {
				for (int i = 0; CheckPlayerState(launcher, 7); i++) {
					if (i > 200)
						break; // 5F) Timeout

					Menu::Global(2698947).At(2).As<int>() = 0;
				}
			}


			SetBit(Menu::Global(2698947).At(1).As<int*>(), 1); // run immediately
			Menu::Global(2698947).At(2).As<int>() = 6; // will change to 7 shortly but that's fine as players are guaranteed not to be in the waiting stage
			*Menu::ScriptLocalStack(launcher->m_stack, 236).At(Native::PlayerId(), 3).At(2).As<int*>() = 6;
			Menu::Global(2698947).At(3).At(1).As<int>() = scriptId;

			launcher->m_context.m_state = Rage::eThreadState::Blocked;
		}
		else {
			Menu::Notify::stacked("Failed To Launch Script 2");
		}
	}
}