...
namespace Menu {
    class PanelManager {
    public:
        PanelManager(float width, float title_height, float item_height) :
            m_Width(width), m_TitleHeight(title_height), m_ItemHeight(item_height)
        {
        }
    public:


        void StartPanel(std::string title);
        void Push(std::string left, std::string right);
        void PushSprite(std::pair<std::string, std::string> texture, Math::Vector2<float> scale);
        void EndPanel();
        void SetNext();

        void SetRenderTitle(bool value) { m_RenderTitle = value; }
    private:
        float m_Width;
        float m_TitleHeight;
        float m_ItemHeight;
        bool m_NextTo = false;
        eFont m_TitleFont = eFont::HouseScript;
        eFont m_ItemFont = eFont::ChaletLondon;
        Color m_Background = { 0, 0, 0, 180 };
        float m_TitleFontScale = 0.3f;
        float m_FontScale = 0.25f;
        Math::Vector2<float> m_Padding = { 3.1f, 2.1f };
        bool m_RenderTitle;
    };

    inline Math::Vector2<float> g_panel_pos = { Framework::GetFrameWork()->m_Position.m_X, Framework::GetFrameWork()->m_Position.m_Y - 0.006f };
    ...
    inline void VehicleInfoPanel(uint32_t hash) {
        PanelManager panel(0.14f, 0, 0.03f);
        panel.StartPanel("");

        if (hash != 0) {
            std::string model = Native::GetDisplayNameFromVehicleModel(hash);
            std::string text = Native::_GetLabelText(model.c_str());
            ...
            auto maxSpeed = Native::_GetVehicleModelMaxSpeed(hash) * 2.236936f;
            auto seats = Native::GetVehicleModelNumberOfSeats(hash);
            auto maxBraking = Native::GetVehicleModelMaxBraking(hash);
            auto maxTraction = Native::GetVehicleModelMaxTraction(hash);
            ...
            if (strlen(VehiclePreviewDictName.c_str()) > 0 && strlen(VehiclePreviewName.c_str()) > 0) {
                ...
                panel.PushSprite({ VehiclePreviewDictName, VehiclePreviewName }, { 0.135f, 0.14f });
