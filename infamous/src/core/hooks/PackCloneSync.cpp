...
namespace Hooks {
	...
	bool PackCloneSyncHook(uint64_t This,Rage::netObject* Object, CNetGamePlayer* Player) {
		if (Object) {
			if (Object->m_object_type >= NetObjectAutomobile && Object->m_object_type < INVALID) {
				if (ProtectionMenuVars::m_Vars.m_Ghosts[Object->m_object_type].m_Enabled) {
					if (Object->m_object_type == NetObjectPlayer) return false;
					if (ProtectionMenuVars::m_Vars.m_Ghosts[Object->m_object_type].m_DisableForSession) return false;
				}
		...
		return OgPackCloneSync(This, Object, Player);
	}
}