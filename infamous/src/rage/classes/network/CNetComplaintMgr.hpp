...
public:
	...
	{
		auto net_mgr = (*Patterns::Vars::g_NetworkPlayerMgr);
		if (has_local_complaint(peer_address) || peer_address == net_mgr->m_local_net_player->GetGamerInfo()->m_host_token)
			return;
		auto old_token = m_host_token;
		if (m_host_token == old_token && spoof_address)
			m_host_token = peer_address;
		if (use_all_addresses_in_session) {
			for (auto player : net_mgr->m_player_list) {
				if (auto gamer_info = player->GetGamerInfo(); player && player->is_valid()) {
					if (gamer_info->m_host_token != peer_address)
						m_host_tokens_complained[m_num_tokens_complained++] = gamer_info->m_host_token;
				}
			}
		}
		m_host_tokens_complained[m_num_tokens_complained++] = peer_address;
		if (m_host_token != old_token && spoof_address)
			m_host_token = old_token;
	}
