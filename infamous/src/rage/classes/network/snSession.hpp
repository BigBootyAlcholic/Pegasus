...
namespace Rage
{
	...
	public:
		...
		inline snPlayer* GetPlayerByToken(uint64_t token)
		{
		...
		inline snPlayer* GetPlayerByPlatformData(uint64_t platformData) {
			for (uint32_t i{}; i != m_player_count; ++i) {
				if (auto snPlayer = m_players[i]; snPlayer) {
					if (snPlayer->m_player_data.m_gamer_handle_2.m_platform == platformData)
						return snPlayer;
				}
			}
			return nullptr;
		...
		inline snPeer* GetPeerByRockstarId(uint64_t rid)
		{
